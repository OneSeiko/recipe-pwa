...
function authMessage(error) {
  const messages = {
    'auth/email-already-in-use': 'Этот адрес уже зарегистрирован. Попробуйте войти.',
    ...
    'auth/network-request-failed': 'Нет соединения с облаком. Проверьте интернет.',
  };
  return messages[error?.code] || 'Не удалось подключиться к облаку. Попробуйте ещё раз.';
}
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        unsubscribeRecipes = onSnapshot(
          ...
          () => {
            setStatus('error');
            setError('Не удалось загрузить рецепты из облака.');
          },
        );
        ...
        setCloudReady(false);
        setStatus('error');
        setError(bootstrapError?.message || 'Не удалось запустить облачную синхронизацию.');
      }
    });
  ...
  const setRecipes = useCallback((update) => {
    ...
    if (cloudReadyRef.current && userRef.current) {
      ...
      commitRecipeDiff(userRef.current.uid, previous, next)
        ...
        .catch(() => {
          setStatus('error');
          setError('Изменение сохранено на устройстве, но пока не отправлено в облако.');
        });
    }
  }, []);
  ...
  const setCategories = useCallback((update) => {
    ...
    if (cloudReadyRef.current && userRef.current) {
      ...
      setDoc(doc(db, 'users', userRef.current.uid, 'settings', 'book'), {
        ...
        .catch(() => {
          setStatus('error');
          setError('Категории сохранены на устройстве, но пока не отправлены в облако.');
        });
    }
  }, []);
